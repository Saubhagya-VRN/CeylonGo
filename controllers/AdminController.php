    ...
    public function reports() {
        ...
        if ($bookingType === 'package' || $bookingType === 'both') {
            ...
            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
                $key = (string)$r['period'];
                if (!isset($rows[$key])) {
                    $rows[$key] = ['period' => $key, 'total' => 0, 'revenue' => 0.0, 'cancelled' => 0];
                }
                $rows[$key]['total']     += (int)$r['total'];
                $rows[$key]['revenue']   += (float)$r['revenue'];
                $rows[$key]['cancelled'] += (int)$r['cancelled'];
            }
        }
 
        // ── Custom trips ─────────────────────────────────────────────────────
        // Revenue = budget_lkr for completed trips.
        // Cancellation = status='cancelled' OR refund_requested_at IS NOT NULL.
        if ($bookingType === 'custom' || $bookingType === 'both') {
            $sql = "
                SELECT
                    {$bucketExpr}                                                           AS period,
        ...
        // Sort by period label ascending
        ksort($rows);
        $rows = array_values($rows);
        ...
        $totalCancellations = array_sum($cancellations);
